...
impl HashIds {
    ...
    pub fn new_with_salt_and_min_length(
        salt: String,
        min_hash_length: usize,
    ) -> Result<HashIds, HashIdsError> {
    ...
    fn _encode(&self, numbers: &Vec<i64>) -> String {
        ...
        if ret_str.len() < self.min_hash_length {
            let guard_idx = (number_hash_int + ret_str.clone().into_bytes()[0] as i64) as usize
                % self.guards.len();
            ...
            if ret_str.len() < self.min_hash_length {
                let guard_idx = (number_hash_int + ret_str.clone().into_bytes()[2] as i64) as usize
                    % self.guards.len();
